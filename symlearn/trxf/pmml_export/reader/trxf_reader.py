...
from symlearn.trxf.classifier import ruleset_classifier
from symlearn.trxf.classifier.ruleset_classifier import RuleSetClassifier
from symlearn.trxf.core import Conjunction, Relation
from symlearn.trxf.pmml_export import models
from symlearn.trxf.pmml_export.models.data_dictionary import Value
from symlearn.trxf.pmml_export.reader import AbstractReader
from symlearn.trxf.pmml_export.models import SimplePredicate, Operator, CompoundPredicate, \
    BooleanOperator
...
class TrxfReader(AbstractReader):
    ...
    def load_data_dictionary(self, X: pd.DataFrame, values: Dict = None):
        ...
        for index, value in dtypes.items():
            ...
            data_fields.append(
                models.DataField(name=str(index), optype=op_type, dataType=data_type, values=wrapped_vals))
